...
test('home growth page no longer shows the book card or check-in controls', () => {
  assert.doesNotMatch(homeTemplate, /class="book-card"/)
  assert.doesNotMatch(homeTemplate, /icon-checkin-calendar-jelly\.png/)
  assert.doesNotMatch(homeTemplate, /bindtap="goCheckinCalendar"/)
  assert.doesNotMatch(homeScript, /goCheckinCalendar\(\)/)
  assert.match(homeTemplate, /本书学习路径/)
})

test('me page hosts check-in calendar entry and metrics', () => {
  assert.doesNotMatch(meTemplate, /message-action\.png/)
  assert.doesNotMatch(meTemplate, /calendar-action\.png/)
  assert.doesNotMatch(meTemplate, /hero-calendar-action/)
  assert.match(meTemplate, /icon-checkin-streak-jelly\.png/)
  assert.match(meTemplate, /icon-checkin-total-jelly\.png/)
  assert.match(meTemplate, /icon-checkin-words-jelly\.png/)
  assert.match(meTemplate, /icon-checkin-minutes-jelly\.png/)
  assert.doesNotMatch(meTemplate, /icon-checkin-today-jelly\.png/)
  assert.match(meTemplate, /坚持打卡，词汇天天涨/)
  assert.match(meTemplate, /bindtap="goCheckinCalendar"/)
  assert.match(meScript, /goCheckinCalendar\(\)/)
  assert.match(meScript, /\/pages\/checkin\/calendar/)
})
...
test('check-in calendar page renders summary, weekday headers, and day cells', () => {
  ...
  assert.match(template, /reward-state-switch/)
  assert.match(template, /wx:for="{{rewardScenarios}}"/)
  assert.match(template, /bindtap="switchRewardScenario"/)
  assert.match(template, /连续不断电/)
  assert.match(template, /开启打卡提醒/)
  assert.match(template, /bindtap="subscribeCheckinRemind"/)
  assert.match(template, /bindchange="toggleCheckinRemindPref"/)
  assert.match(template, /checkinRemindEnabled/)
  assert.match(template, /提醒已关闭/)
  assert.match(template, /power-remind-hint/)
  ...
  assert.match(template, /day-cell-reward-claimable/)
  assert.match(template, /giftClaimed \? '已领' : \(giftUnlocked \? '可领' : '30天'\)/)
  ...
  assert.match(style, /\.reward-state-switch\s*{/)
  assert.match(style, /\.reward-state-chip-active/)
  ...
  assert.match(style, /\.day-cell-reward-claimable/)
...
test('check-in calendar unlocks gift reward after 30 continuous days', () => {
  ...
  assert.match(calendarScript, /REWARD_SCENARIOS/)
  assert.match(calendarScript, /label:\s*'未达标'/)
  assert.match(calendarScript, /label:\s*'可领取'/)
  assert.match(calendarScript, /label:\s*'已领取'/)
  assert.match(calendarScript, /switchRewardScenario\(event\)/)
  assert.match(calendarScript, /applyRewardScenario/)
  assert.match(calendarScript, /giftUnlocked: displayContinuousDays >= STREAK_REWARD_DAYS/)
  assert.match(calendarScript, /giftClaimed:\s*false/)
  assert.match(calendarScript, /pickRewardClaimed\(info\)/)
...
test('check-in calendar gift dialog has locked remaining and copied states', () => {
  ...
  assert.match(template, /wx:if="{{giftClaimed}}"/)
  assert.match(template, /wx:elif="{{giftUnlocked}}"/)
  assert.match(template, /立即领取/)
