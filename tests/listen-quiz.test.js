...
test('listen page renders a home-styled fill-in quiz with top progress and new words', () => {
  ...
  assert.match(listenStyle, /\.listen-quiz-top-progress-fill\s*{[^}]*background:\s*#4dd9a0/s)
...
test('quiz hint action aligns with the word-new hint placement', () => {
  ...
  assert.match(listenStyle, /\.quiz-bottom-area\s*{[^}]*padding-bottom:\s*calc\(env\(safe-area-inset-bottom\) \+ 48rpx\)/s)
