...
test('finish today page surfaces check-in success after reaching the daily goal', () => {
  ...
  assert.match(script, /getStageInfo/)
  assert.match(script, /buildContinueUrl/)
  assert.match(script, /continueLabel/)
  assert.match(script, /wx\.redirectTo/)
  assert.match(script, /returnTabUrl/)
  assert.match(script, /normalizeReturnTab/)
  ...
  assert.match(script, /headerImageForScoreRate/)
  assert.match(script, /scoreRate/)
  assert.match(template, /\{\{headerImage\}\}/)
  assert.match(template, /\{\{continueLabel\}\}/)
  assert.match(template, /wx:if="\{\{showContinue\}\}"/)
  assert.match(style, /\.checkin-progress-line/)
  assert.match(style, /\.checkin-success-card/)
  assert.match(style, /\.checkin-calendar-btn/)
  assert.match(style, /\.finish-header/)
})
