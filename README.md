...
> 版本 `0.6.1` · 一款面向中小学生的英语词汇学习小程序，融合关卡地图、背单词、随身听、背诵跟读、打卡与学习计划。
...
- 大图资源迁至 Vercel CDN，新增 `utils/image-host.js` 远程加载
- 首页、随身听、打卡分享、完成页等接入远程图片
- 学习记录页样式与数据展示优化
- 微信上传包 `project.config.json` 忽略规则更新

### v0.6.0

- 学习记录页改版：详情卡片、统计头图与 Jelly 图标资源
- 已购词书页布局与交互优化
- 学习记录数据层 `utils/study-records.js` 扩展
- 首页与我的页面样式微调，补充相关测试

### v0.5.0

- 还原批量压缩受损的图片资源（PK 雪碧图、加载动画、头图、分享海报等）为清晰 PNG
- 完成页头图、打卡分享背景、App Logo 等改回无损 PNG 引用
- 修复 `audio.png` 喇叭图标与练习页路线图标引用
- 微信上传包优化：`.git` 外置、`packOptions.ignore` 与构建素材迁至 `assets/`
- 图片源文件与构建脚本整理（`assets/pk-build/`、`scripts/build-*.py` 更新）

### v0.2.8

- 小程序图标与 App Logo 资源（1024 多版本 + 背景修复脚本）
- 打卡分享海报浅色背景、Logo 与导出优化
- 随身听进度条 Jelly 滑块 v10–v13 多风格预览
- 广告页扁平图标、产品主图与套餐 Jelly 素材
- 随身听单词/句子标签、背单词与首页样式调整
- 新增 `utils/app-brand.js` 与多项素材构建脚本
